...
void run_cli_mode() {
    ...
    printf("Empty space is represented by '_', shapes by '*'.\n\n");
    ...
    while (1) {
        ...
        switch (choice) {
            case 1: {
                ...
                printf("\n--- Add Line ---\n");
                int x1 = read_int("Enter X1: ");
                int y1 = read_int("Enter Y1: ");
                int x2 = read_int("Enter X2: ");
                int y2 = read_int("Enter Y2: ");
            ...
            case 2: {
                ...
                printf("\n--- Add Rectangle ---\n");
                int x = read_int("Enter bottom-left corner X: ");
                int y = read_int("Enter bottom-left corner Y: ");
                int w = read_int("Enter width: ");
                int h = read_int("Enter height: ");
                ...
                shapes[num_shapes].data.rect.x = x;
                shapes[num_shapes].data.rect.y = y;
                shapes[num_shapes].data.rect.width = w;
                shapes[num_shapes].data.rect.height = h;
                num_shapes++;
            ...
            case 3: {
                ...
                printf("\n--- Add Circle ---\n");
                int cx = read_int("Enter center X: ");
                int cy = read_int("Enter center Y: ");
                int r = read_int("Enter radius: ");
            ...
            case 4: {
                ...
                int x2 = read_int("Enter X2: ");
                int y2 = read_int("Enter Y2: ");
                int x3 = read_int("Enter X3: ");
                int y3 = read_int("Enter Y3: ");
    ...
    for (int i = 0; i < num_shapes; i++) {
        ...
        switch (s.type) {
            case SHAPE_LINE:
                ...
                break;
            case SHAPE_RECTANGLE:
                printf("Rectangle: Bottom-Left=(%d, %d), Width=%d, Height=%d\n",
...
int pick_coordinates(int *out_x, int *out_y, int has_preview, Shape temp_shape) {
    ...
    while (1) {
        if (has_preview) {
            Shape preview_shape = temp_shape;
            switch (temp_shape.type) {
                case SHAPE_LINE:
                    preview_shape.data.line.x2 = cur_x;
                    preview_shape.data.line.y2 = cur_y;
                    break;
                case SHAPE_RECTANGLE: {
                    int x1 = temp_shape.data.rect.x;
                    int y1 = temp_shape.data.rect.y;
                    preview_shape.data.rect.x = x1 < cur_x ? x1 : cur_x;
                    preview_shape.data.rect.y = y1 < cur_y ? y1 : cur_y;
                    preview_shape.data.rect.width = abs(cur_x - x1) + 1;
                    preview_shape.data.rect.height = abs(cur_y - y1) + 1;
                    break;
                }
                case SHAPE_CIRCLE: {
                    ...
                    preview_shape.data.circle.r = (int)(dist + 0.5);
                    break;
                }
                case SHAPE_TRIANGLE:
                    preview_shape.data.triangle.x3 = cur_x;
                    preview_shape.data.triangle.y3 = cur_y;
                    break;
            }
            render_preview_canvas(preview_shape);
        } else {
            render_canvas();
        }
